...
In this fork has been added the following CLI command to `pbi-tools.exe` (Desktop version):

#### extract-model

This command is based on the `extract` command and should be used to extract only the data model, the arguments are the same as the `extract` command except `-watch` argument, the output folder generated by this command only generates the `Model` folder.

**NOTE** In the extract command code base there is an initiative to have the argument `-parts`, when this is implemented in the official repo this fork is no longer necessary.

This [issue](https://github.com/pbi-tools/pbi-tools/issues/194) has been raised to implement the `-parts` CLI option.
